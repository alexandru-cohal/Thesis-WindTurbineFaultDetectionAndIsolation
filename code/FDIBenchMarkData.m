...
% Ts = 1 / 100;
Lungime_Fereastra = 512; % pentru FFT (Fault_1, Fault_3, Fault_4)
% Tip_Fereastra = 'hann';
% Suprapunere = 75 / 100;
NFFT = 512; % Numar puncte FFT (Fault_1, Fault_3, Fault_4)
Lungime_Fereastra_2 = 5; % pentru medie (Fault_1, Fault_3, Fault_4)
Lungime_Fereastra_3 = 100; % pentru autocorelatie (Fault_5)

% Proiectare estimator Tau_g (Fault_8)
alpha_gc = 50;
num = [alpha_gc];
den = [1 alpha_gc];
[A, B, C, D] = tf2ss(num, den);
vpe = [-100];  % Valorile proprii alocate estimatorului (de 2 ori mai indepartate)
L = acker(A', C', vpe)'; % Matricea estimatorului

% Proectare filtru reziduu Tau_g (Fault_8)
Fordin = 6;
Fcut_off = 0.004;
[Fb, Fa] = butter(Fordin, Fcut_off);